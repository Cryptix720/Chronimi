...
/*
   ...
   * (NPM) Install - Will ask for where to put chronimi (outside pm folder)
   * (NPM) Upgrade - Will look for chronimi install, copy over files and update if new version
...
var
  currentConfig = requireDotFile('chronimi.json'),
...
// run update scripts if chronimi.json exists
if(currentConfig && manager.name === 'NPM') {
  ...
  if( fs.existsSync(updatePaths.definition) ) {
    ...
    if(currentConfig.version !== release.version) {
      console.log('Updating chronimi UI from ' + currentConfig.version + ' to ' + release.version);
      ...
      // update version number in chronimi.json
      ...
      console.log('Current version of chronimi UI already installed');
      return;
    }
...
gulp.task('run setup', function() {
  ...
  // If auto-install is switched on, we skip the configuration section and simply reuse the configuration from chronimi.json
  if(install.shouldAutoInstall()) {
    answers = {
      overwrite    : 'yes',
      install      : 'auto',
      useRoot      : true,
      chronimiRoot : currentConfig.base
    };
  }
  else {
    return gulp
...
gulp.task('create install files', function(callback) {
  ...
  if(manager && (answers.useRoot || answers.customRoot)) {
    ...
    // add project root to chronimi root
    installFolder = path.join(manager.root, answers.chronimiRoot);
    ...
    console.log('Installing to \x1b[92m' + answers.chronimiRoot + '\x1b[0m');
  ...
  gulp.task('create chronimi.json', function() {
    ...
    if( fs.existsSync(installPaths.config) ) {
      console.info('Extending config file (chronimi.json)', installPaths.config);
      ...
      console.info('Creating config file (chronimi.json)', installPaths.config);
...
gulp.task('clean up install', function() {
  ...
  if(installFolder && !install.shouldAutoInstall()) {
    console.log('\n Setup Complete! \n Installing Peer Dependencies. \x1b[0;31mPlease refrain from ctrl + c\x1b[0m... \n After completion navigate to \x1b[92m' + answers.chronimiRoot + '\x1b[0m and run "\x1b[92mgulp build\x1b[0m" to build');
    process.exit(0);
  }
  else {
    console.log('');
